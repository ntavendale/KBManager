...
constructor TKBLoginSettings.Create(AIniFileName: String = '');
begin
  inherited Create;
...
procedure TKBLoginSettings.Load;
begin
  ...
  try
    var LSection := 'DBLogin';
    if LIniFile.SectionExists('KBLogin') then
      LSection :=  'KBLogin';
...
procedure TKBLoginSettings.Save;
begin
  var LIniFile := TIniFile.Create(FIniFile);
  try
    var LSection := 'KBLogin';
...
class procedure TKBLoginSettings.LoadFromJsonString(AJsonString: String);
var
  LJson: TJsonObject;
  LPassword: String;
begin
